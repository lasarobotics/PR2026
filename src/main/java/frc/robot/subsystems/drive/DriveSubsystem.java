...
public class DriveSubsystem extends StateMachine {
  public enum DriveStates implements SystemState {
          ...
          s_drivetrain.setControl(
              s_drive
                  ...
                  .withVelocityY(
                      Constants.DriveConstants.MAX_SPEED
                          .times(
                              -s_driveRequest.getAsDouble()
                                  * Math.abs(s_driveRequest.getAsDouble()))
                          .times(s_currentSpeedScalar))
        ...
        if (!s_isClimbing) {
          double pidOutputAngle =
              getInstance()
                  ...
                  .calculate(s_currentRotation, s_desiredRotation);
    ...
    FULL_CLIMB_ALIGN {
      ...
      public void execute() {
        if (!s_isClimbing) {
          ...
          // Constants.DriveConstants.MAX_ANGULAR_RATE.times(pidOutputAngle).in(RadiansPerSecond);
          // pidInput = pidInput > 0 ? Math.min(pidInput, 8.0) : Math.max(pidInput, -8.0);
          Logger.recordOutput("ClimbPOS", climbPos);
        }
      }
    ...
    PARK {
      ...
      public void initialize() {
        ...
        getInstance().m_module0DrivePos =
            s_drivetrain.getModule(0).getDriveMotor().getPosition().getValueAsDouble();
        getInstance().m_module1DrivePos =
            s_drivetrain.getModule(1).getDriveMotor().getPosition().getValueAsDouble();
        getInstance().m_module2DrivePos =
            s_drivetrain.getModule(2).getDriveMotor().getPosition().getValueAsDouble();
        getInstance().m_module3DrivePos =
            s_drivetrain.getModule(3).getDriveMotor().getPosition().getValueAsDouble();
      }
  ...
  public double getDistanceToHub() {
    ...
    double distanceToHub =
        Math.sqrt(Math.pow(differenceFromHub.getX(), 2) + Math.pow(differenceFromHub.getY(), 2));
    return distanceToHub;
  }

  public double getPredictedDistanceToHub() {
    SwerveDriveState currentState = s_drivetrain.getState();
    Translation2d velocityTranslation =
        new Translation2d(
                currentState.Speeds.vxMetersPerSecond * Constants.DriveConstants.FUEL_AIR_TIME,
                currentState.Speeds.vyMetersPerSecond * Constants.DriveConstants.FUEL_AIR_TIME)
  ...
  public boolean awayFromTower() {
    if (Math.abs(
            Constants.DriveConstants.CENTER_XPOS - DriveSubsystem.getInstance().getPose().getX())
        < Constants.DriveConstants.STOW_DISTANCE_REQUIREMENT) return true;
    return false;
  }
  ...
  public static class ParkRequest implements SwerveRequest {
    public StatusCode apply(
        SwerveControlParameters paraeters, SwerveModule<?, ?, ?>... modulestoApply) {
      // front left
      s_drivetrain
          .getModule(0)
          .apply(
              new PositionVoltage(getInstance().m_module0DrivePos).withSlot(1),
              new PositionVoltage(0.125));
      // front right
      s_drivetrain
          .getModule(1)
          .apply(
              new PositionVoltage(getInstance().m_module1DrivePos).withSlot(1),
              new PositionVoltage(-0.125));
      // back left
      s_drivetrain
          .getModule(2)
          .apply(
              new PositionVoltage(getInstance().m_module2DrivePos).withSlot(1),
              new PositionVoltage(-0.125));
      // back right
      s_drivetrain
          .getModule(3)
          .apply(
              new PositionVoltage(getInstance().m_module3DrivePos).withSlot(1),
              new PositionVoltage(0.125));
